 <?php
  ...
  $sql = "SELECT VD.*, C1.nombre_ciudad AS origen, C2.nombre_ciudad AS destino, 
    P1.nombre_provincia AS origenProvincia, P2.nombre_provincia AS destinoProvincia, P.*, C.nombre_cooperativa, B.* 
    FROM Viajes_Diarios AS VD JOIN Frecuencias_Cooperativas AS FC ON FC.id_asignacion = VD.id_asignacion_pertenece
    ...
    JOIN Buses AS B ON B.id_socio = U.id_usuario WHERE F.tipo_frecuencia = '$tipo_viaje'  
    AND C1.nombre_ciudad = '$origen' AND C2.nombre_ciudad = '$destino' GROUP BY P.id_parada";  
